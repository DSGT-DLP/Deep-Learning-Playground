...
def enumclass(cls=None, /, *, DataClass: BaseData = None, **kwargs):
    ...
    def process(cls):
        ...
        setattr(
            cls,
            "Attribute",
            _BaseEnum("Attribute", [(field.upper(), field) for field in data_fields]),
        )

        for attribute in kwargs:
            if attribute in data_fields:
                enum_name = "".join(
                    map(lambda string: string.capitalize(), attribute.split("_"))
                )
                setattr(
                    cls,
                    enum_name,
                    _BaseEnum(
                        ...
                        [
                            ...
                            for element in kwargs[attribute]
                        ],
                    ),
                )
            else:
...
def changevar(
    ...
    *,
    ...
    if (
        cls is not None
        or DataClass is None
        or EnumClass is None
...
class BaseDDBUtil:
    ...
    def create_table(
        self, read_capacity_units: int = 10, write_capacity_units: int = 10
    ) -> None:
            ...
            AttributeDefinitions=[
                {
                    ...
                    "AttributeType": _type_mapper[
                        self.DataClass.__dataclass_fields__[self.partition_key].type
                    ],
                }
            ],
    ...
    def create_gsi(
        self,
        attribute_name: str,
        attribute_type: str = "HASH",
        projection_type: str = "KEYS_ONLY",
        read_capacity: int = 10,
        write_capacity: int = 10,
        nonkey_attributes: List[str] = None,
    ) -> Literal["Success"]:
        """Function that adds a global secondary index to the associated table"""

        if (
            type(attribute_name) != str
            or type(attribute_type) != str
            or type(read_capacity) != int
            or type(write_capacity) != int
            or type(projection_type) != str
        ):
            raise ValueError(
                "Cannot create global secondary index with invalid argument"
            )

        if attribute_name not in self.EnumClass.Attribute:
            raise ValueError(
                f"Attribute '{attribute_name}' not found in table {self.table_name}"
            )
        ...
        if projection_type == "INCLUDE":
            if nonkey_attributes is None:
                raise ValueError(
                    f"nonkey_attributes need to be provided for projection_type 'INCLUDE'"
                )
            else:
                for attribute in nonkey_attributes:
                    if attribute not in self.EnumClass.Attribute:
                        raise ValueError(
                            f"Attribute '{attribute}' not found in table {self.table_name}"
                        )
        elif nonkey_attributes is not None:
            raise ValueError(
                f"nonkey_attributes should not be provided for projection_type 'f{projection_type}'"
            )
            ...
            AttributeDefinitions=[
                {
                    ...
                    "AttributeType": _type_mapper[
                        self.DataClass.__dataclass_fields__[attribute_name].type
                    ],
                }
            ],
    ...
    def create_record(
        self, record_data: BaseData = None, **kwargs
    ) -> Literal["Success"]:
        ...
        try:
            ...
            if e.response["Error"]["Code"] == "ConditionalCheckFailedException":
                raise ValueError(
                    f"Could not add record. {partition_key_name} {item[partition_key_name]} already exists in the table"
                )
            else:
                raise e
    ...
    def get_record(self, partition_id: Any) -> BaseData:
        ...
        if "Item" not in response:
            raise ValueError(
                f"Could not find a DynamoDB item for {self.partition_key} {partition_id} in table {self.table_name}"
            )
    ...
    def update_record(self, partition_id: Any, **kwargs) -> Literal["Success"]:
        ...
        try:
            self.table.update_item(
                Key={partition_key_name: partition_id},
            ...
            if e.response["Error"]["Code"] == "ConditionalCheckFailedException":
                raise ValueError(
                    f"Could not find a DynamoDB item to update for {self.partition_key} {partition_id} in table {self.table_name}"
                )
            else:
                raise e
    ...
    def delete_record(self, partition_id: Any) -> Literal["Success"]:
        ...
        try:
            ...
            if e.response["Error"]["Code"] == "ConditionalCheckFailedException":
                raise ValueError(
                    f"Could not find a DynamoDB item to delete for {self.partition_key} {partition_id} in table {self.table_name}"
                )
            else:
                raise e
    ...
    def __param_checker(self, operation: str, **kwargs):
        ...
        for attribute, value in kwargs_items:
            if attribute == "record_data":
                if value is not None:
                    if type(value) is not self.DataClass:
                        raise ValueError(
                            f"Could not {operation} record with {attribute} not of correct type"
                        )
                    if len(kwargs) > 1:
                        raise ValueError(
                            f"Cannot provide other attributes if {attribute} is provided"
                        )
                    self.__param_checker(operation, **asdict(value))
                elif len(kwargs) == 1:
                    raise ValueError(
                        f"Could not {operation} record with missing attributes"
                    )
                continue
                ...
                if partition_key_name in kwargs:
                    raise ValueError(
                        f"Cannot {operation} record with multiple values for {partition_key_name}"
                    )
            ...
            if attribute not in self.EnumClass.Attribute:
                ...
                self.EnumClass, attribute.capitalize(), None
            ) is not None and value not in getattr(
                self.EnumClass, attribute.capitalize()
            ):
                raise ValueError(
                    f"Could not {operation} record with invalid {attribute}: {value}"
                )
                ...
                if key in self.EnumClass.Attribute:
                    ...
                    if type(value) != key_type:
                        if value != key_type(value):
                            raise ValueError(
                                f"Could not approve record with {key} not of correct type"
                            )
                        item[key] = key_type(value)
                else:
                    raise ValueError(
                        f"Attribute {key} not found in table {self.table_name}"
                    )
            pass
        return item
