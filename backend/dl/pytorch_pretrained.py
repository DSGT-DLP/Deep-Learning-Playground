...
def pytorch_pretrained(n_class, model_name, num_epochs, device, in_chan, loss_func, train_loader, valid_loader, optimizer_name="ADAM", send_progress= lambda x: print("send progerss", x)):
    ...
    return train_deep_image_classification(model, train_loader, valid_loader, optimizer, loss_func, num_epochs, device, send_progress)
...
class GetModel(nn.Module):
    def __init__(self, model_name, N_CLASS, in_chan, pretrained = True):
        ...
        self.cnn = timm.create_model(self.model_name, pretrained = pretrained, num_classes = N_CLASS, in_chans =in_chan)
...
if __name__ == "__main__":
  ...
  train_pretrained(n_class=2, model_name='inception_v3', num_epochs=2, split_fraction=0.2, device=torch.device('cpu'), in_chan=3, loss_func="CELOSS", train_loader=train_loader, valid_loader=test_loader)
