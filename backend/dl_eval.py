...
def compute_accuracy(predicted, actual):
    ...
    """
    prediction = torch.argmax(predicted, dim=1) #identify index of the most likely class 
    
    performance = torch.where((prediction == actual), torch.Tensor([1.0]), torch.Tensor([0.0])) #for each row, did you predict correctly?
