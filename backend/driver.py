...
def drive(user_arch: list[str], criterion: str, optimizer_name: str,         problem_type: str, target: str = None,
          default: bool = False, test_size: float = 0.2, epochs: int = 5, shuffle: bool = True):
    """
    ...
    if (default):
        ...
        input_df.columns = ['sepal_len', 'sepal_wid', 'petal_len', 'petal_wid',
                            'class']
    ...
    X_train_tensor = torch.reshape(X_train_tensor, (
        X_train_tensor.size()[0], 1, X_train_tensor.size()[1]))
    y_train_tensor = torch.reshape(y_train_tensor,
                                   (y_train_tensor.size()[0], 1))
    ...
    X_test_tensor = torch.reshape(X_test_tensor, (
        X_test_tensor.size()[0], 1, X_test_tensor.size()[1]))
    ...
    print(f"model: {model}")
    optimizer = get_optimizer(model, optimizer_name=optimizer_name,
                              learning_rate=0.05)
    criterion = LossFunctions.get_loss_obj(LossFunctions[criterion])
    print(f"loss criterion: {criterion}")
    train_loader, test_loader = get_dataloaders(X_train_tensor, y_train_tensor,
...
if __name__ == "__main__":
    print(1111)
    # drive(["nn.Linear(4, 10)", "nn.Linear(10, 3)"], "CELOSS", "SGD",
    #       problem_type="classification", default=True, epochs=10)
