...
class StatusDDBUtil:
    ...
    def __init__(self, table_name: str, region: str, table=None):
        ...
        self.table = (
            table
            if table
            else boto3.resource("dynamodb", region).Table(self.table_name)
        )
    ...
    def get_record(self, request_id: str) -> StatusData:
        ...
        response = self.table.get_item(
            Key={StatusAttribute.REQUEST_ID.value: request_id}
        )
    ...
    def update_status(self, request_id: str, new_status: StatusEnum):
        ...
        try:
            ...
            raise ValueError(
                f"Oops. Could not update status to {new_status} for request id {request_id}"
            )
