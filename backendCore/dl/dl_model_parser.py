...
def parse_deep_user_architecture(user_model):
    """
    new:
    Each layer in the user_model has this format:
    {"value": "LAYER_NAME", "parameters": [PARAMETER_1, PARAMETER_2, ...]}

    Eg: [{"value": "LINEAR", "parameters": [50, 10]}] should become [nn.Linear(in_features=50, out_features=10)]
    ...
    for element in user_model:
        if element["value"] == "LINEAR":
            layer = nn.Linear(
                in_features=element["parameters"][0],
                out_features=element["parameters"][1],
            )
        elif element["value"] == "RELU":
            layer = nn.ReLU()
        elif element["value"] == "SOFTMAX":
            layer = nn.Softmax(dim=element["parameters"][0])
        else:
