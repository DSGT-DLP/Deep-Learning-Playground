...
import { Box, Button, Flex, IconButton, Dropdown, PageHeader, Spinner } from 'gestalt'
...
ChartJS.register(ArcElement, Tooltip, Legend, LinearScale, BarElement, Title, TimeSeriesScale)
...
const BlankGrid = () => {
  ...
  return (
    <div id='blank-grid-wrapper'>
      <div id='blank-grid'>
        <p>You haven't trained any models yet. Create your first model below!</p>
...
const StatusDisplay = ({ statusType, status }) => {
  ...
  if (statusType === 'QUEUED') {
    ...
    return (
      <button className='grid-status-display grid-status-display-red' onClick={() => navigate('/')}>
        Error
      </button>
    )
...
const formatDate = (date) => {
  ...
  return (
    ...
    date.toLocaleDateString(undefined, {
      month: 'short',
      day: 'numeric',
      year: date.getFullYear() === currDate.getFullYear() ? undefined : 'numeric',
    })
  )
}
...
const FilledGrid = (props) => {
  ...
  async function handleOnDownloadClick(e, row) {
    ...
    zip.generateAsync({ type: 'blob' }).then((blob) => saveAs(blob, 'results.zip'))
  }
  return (
    <>
      {executionTable ? (
              ...
              {executionTable.map((row) => (
                ...
                >
                  ...
                  <TableCell align='left'>
                    <StatusDisplay statusType={row.status} status={`${row.progress.toFixed(2)}%`} />
                  </TableCell>
                  <TableCell align='left'>
                    <IconButton
                      icon='download'
  ...
  return (
    <div id='dashboard'>
      <>
        ...
        />
        <Flex direction='row' justifyContent='center' alignItems='stretch' width='100%' wrap>
                  ...
                  plugins: {
                    tooltip: {
                      callbacks: {
                        title: (context) => {
                          return format(
                            execFrequencyBarData.datasets[0].data[context[0].dataIndex].x,
                            'MMM d',
                          )
                        },
                      },
