...
const Results = (props) => {
  ...
  const auc_roc_data_res = dlpBackendResponse?.auxiliary_outputs?.AUC_ROC_curve_data || []
  const auc_roc_data = []
  ...
  const dl_results_columns_react_csv = Object.keys(dl_results_data[0] || []).map((c) => ({
    label: c,
    key: c,
  }))

  const mapResponses = (key) => dlpBackendResponse?.dl_results?.map((e) => e[key]) || []
  ...
  return (
    <>
      {choice === 'objectdetection' ? <img src={`data:image/jpeg;base64,${image_data}`} /> : null}
        ...
        <div style={{ marginTop: 8 }}>
          ...
          {problemType.value === 'classification' && auc_roc_data_res.length === 0 ? (
            <p style={{ textAlign: 'center' }}>
              No AUC/ROC curve could be generated. If this is not intended, check that shuffle is
              set to true to produce a more balanced train/test split which would enable correct AUC
              score calculation
            </p>
          ) : null}
