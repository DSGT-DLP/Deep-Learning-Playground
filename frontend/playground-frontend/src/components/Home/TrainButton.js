...
import { sendEmail, uploadToBackend, train_and_output } from '../helper_functions/TalkWithBackend'
...
const TrainButton = (props) => {
  ...
  const make_obj_param_list = (obj_list, source) => {
    ...
    for (let i = 0; i < obj_list.length; i++) {
      ...
      let is_transform_type =
        obj_list_item.transform_type && obj_list_item.transform_type === 'functional'
  ...
  const onClick = async () => {
    ...
    const trainState = await train_and_output(choice, functionMap[choice][1](paramList))
