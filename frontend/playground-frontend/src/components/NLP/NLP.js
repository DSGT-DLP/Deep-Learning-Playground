...
const NLP = () => {
    ...
    const onClick = async () => {
        ...
        output = await postData('/wordVec', { 'raw_review': nlpData,'inputWord':inputWord, 'vectorSize': vectorSize, 'windowSize': windowSize, 'minCount': minCount, 'workers': workers });
    ...
    return (
        <>
            <div style={{ padding: 20 }}>
                ...
                <span style={{ marginLeft: 8, visibility: (submitted ? 'visible' : 'hidden') }}>
                    <a onClick={() => { console.log(blobUrl); }} href={blobUrl} download="word2vec.model" style={styles.download_csv_res}>
