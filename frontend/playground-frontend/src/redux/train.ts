    ...
    handleFileUpload: (state, action: PayloadAction<File>) => {
      const csvToJson = (dataString: string) => {
        ...
        const columns = headers.map((c) => ({
          name: c,
          selector: (row: { [x: string]: any }) => row[c],
        }))
        return [list, columns]
      }
      ...
      state.fileName =
        file.name == null
          ? undefined
          : file.name.replace(/\.[^/.]+$/, '').substring(0, 20) + '.' + file.name.split('.').pop()
...
export const { setModelName, resetTrain, handleFileUpload } = trainSlice.actions
